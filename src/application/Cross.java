...
public class Cross extends PrimitiveObstacle {
	//private ImageView obImg;
	public Cross() {
		Image RingObstacle = new Image("model/resources/Cross.png");
		obImg = new ImageView(RingObstacle);
		obImg.setFitHeight(280);
		obImg.setFitWidth(280);
		obImg.setLayoutX(120);
		obImg.setLayoutY(30);
	}
	@Override
	...
	public boolean collision(Circle ball){
		...
		if(pr!=null) {
			Color color = pr.getColor((int)(((ball.getLayoutX()-this.getImg().getLayoutX())/(this.getImg().getFitWidth())) * image.getWidth()), (int)(((ball.getLayoutY()-this.getImg().getLayoutY())/(this.getImg().getFitHeight())) * image.getHeight()));
